...
func SearchExperimentForPublication(c *fiber.Ctx, pool *pgxpool.Pool) error {
	...
	query, err := sql.SQLf(`
		SELECT 
			json_agg(json_build_object('id',join_publication_exp.exp_id,'metadata',join_publication_exp.metadata)) as exps,
		...
		-- TO ONLY SELECT THE PUBLICATION THAT HAVE ALL OF THEIR EXPERIMENT ALREADY PROCESS BY NIMBUS
			--INNER JOIN (
			--select 
		...
		ON join_publication_exp.publication_id = table_publication.id AND join_publication_exp.exp_id is not NULL %s
	...
	type Response struct {
		Exps          []map[string]any `json:"exps" sql:"exps"`
